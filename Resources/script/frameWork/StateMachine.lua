...
function CreateStateMachine()
	...
	function StateMachine:onError_(event, error, message)
		cclog("---StateMachine Do Using Event and it's not a error---");
		-- cclog("[StateMachine] ERROR: %s, event %s, from %s to %s", tostring(error), event.name, event.from, event.to);
		-- cclog("%s [StateMachine] ERROR: error %s, event %s, from %s to %s", tostring(self.target_), tostring(error), event.name, event.from, event.to)
	end
